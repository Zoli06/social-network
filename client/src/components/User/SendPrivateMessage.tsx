...
export const SendPrivateMessage = ({ user }: SendPrivateMessageProps) => {
  ...
  const [sendPrivateMessage] = useMutation(SEND_PRIVATE_MESSAGE_MUTATION, {
    update(cache, { data: { sendPrivateMessage: privateMessage } }) {
      cache.modify({
        id: cache.identify(user),
        fields: {
          myPrivateMessagesWithUser(existingPrivateMessages = []) {
            const newPrivateMessageRef = cache.writeFragment({
              data: privateMessage,
              fragment: PrivateMessage.fragments.privateMessage,
              fragmentName: 'PrivateMessage',
            });

            return [...existingPrivateMessages, newPrivateMessageRef];
          },
        },
      });
    },
